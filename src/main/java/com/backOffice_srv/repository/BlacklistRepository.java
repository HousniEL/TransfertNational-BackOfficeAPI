...
public interface BlacklistRepository extends JpaRepository<Blacklist,Integer> {
    ...
    @Query(value = "SELECT * FROM blacklist where first_name=?1 and last_name=?2 and phone_number=?3 and archived = 0 limit 1",
            nativeQuery = true)
    ...
    @Query(value = "SELECT count(*) FROM blacklist where archived = 0",
            nativeQuery = true)
    Integer countBlacklist();

    @Query(value = "SELECT * FROM blacklist where phone_number=?1 and archived = 0 limit 1",
            nativeQuery = true)
